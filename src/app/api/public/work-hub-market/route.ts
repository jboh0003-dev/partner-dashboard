...
async function fetchChart(symbol: string) {
  ...
  const price = [...closes].reverse().find((v) => typeof v === "number") ?? meta.regularMarketPrice ?? null;
  const previousClose = meta.chartPreviousClose ?? meta.previousClose ?? null;
  const change = typeof price === "number" && typeof previousClose === "number" ? price - previousClose : null;
  const changePct = typeof change === "number" && typeof previousClose === "number" && previousClose !== 0 ? (change / previousClose) * 100 : null;
  return { symbol, price, previousClose, change, changePct, currency: meta.currency ?? null };
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { ok: true, updatedAt: new Date().toISOString(), kospi, usdkrw, stocks },
      { headers: { "Cache-Control": "no-store, max-age=0" } },
    );
  } catch (error) {
    console.error("[work-hub-market]", error);
    return NextResponse.json({ ok: false, message: "시장 데이터를 불러오지 못했습니다." }, { status: 502 });
  }
}
