...
export function PartnerApplyWizard({ initialId, initialToken }: Props) {
  ...
  function goMissing() {
    const first = (missing.length ? missing : liveMissing)[0];
    if (!first) return;
    ...
    setStep(map[first.section] || "review");
  }

  const canSubmit = liveMissing.length === 0 && editable;
      ...
      <div className="mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        {lookupPassword ? (
          <p className="mt-2 text-sm text-slate-600">
            조회 비밀번호: <strong>{lookupPassword}</strong> (이 화면을 닫기 전에 저장하세요)
          </p>
        ) : null}
        <p className="mt-4 text-sm text-slate-500">
          검토 결과는 담당자 연락처로 안내됩니다. 신청 상태 조회는 아래 링크로 가능합니다.
        </p>
        <Link href="/partner-apply" className="mt-6 inline-block text-sm text-blue-700 underline">
          신청 포털로 돌아가기
        </Link>
      </div>
    );
        ...
        {step === "intro" ? (
          <div className="space-y-4">
            ...
            <ul className="list-disc space-y-1 pl-5 text-sm text-slate-600">
              ...
              <li>기술협력 또는 플래티넘 검토 신청 시 기술인력·장비가 필수로 전환됩니다.</li>
        ...
        {step === "review" ? (
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-600">
              누락 항목: <strong>{liveMissing.length}</strong>개
            </p>
            {liveMissing.length ? (
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <label className="text-sm">
          기업명 <RequiredMark />
          <input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.company_name} onChange={(e) => set("company_name", e.target.value)} />
        ...
        <label className="text-sm">
          대표자명 <RequiredMark />
          <input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.representative_name} onChange={(e) => set("representative_name", e.target.value)} />
          <FieldError show={hasMiss(missing, "representative_name")} message="대표자명을 입력하세요." />
        </label>
        <label className="text-sm">
          설립일자 <RequiredMark />
          <input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" placeholder="YYYY-MM-DD 또는 2021년 1월" value={c.established_date} onChange={(e) => set("established_date", e.target.value)} />
          <FieldError show={hasMiss(missing, "established_date")} message="설립일자를 입력하세요." />
        </label>
        <label className="text-sm sm:col-span-2">
          주소 <RequiredMark />
          <input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.address} onChange={(e) => set("address", e.target.value)} />
        ...
        <label className="text-sm">전체 임직원 수 <RequiredMark /><input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.total_employees} onChange={(e) => set("total_employees", e.target.value)} /><FieldError show={hasMiss(missing, "total_employees")} message="필수" /></label>
        <label className="text-sm">전체 엔지니어 수 <RequiredMark /><input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.total_engineers} onChange={(e) => set("total_engineers", e.target.value)} /><FieldError show={hasMiss(missing, "total_engineers")} message="필수" /></label>
        <label className="text-sm">오케스트로 전담 영업인원 수 <RequiredMark /><input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.dedicated_sales_count} onChange={(e) => set("dedicated_sales_count", e.target.value)} /><FieldError show={hasMiss(missing, "dedicated_sales_count")} message="필수" /></label>
        <label className="text-sm">오케스트로 전담 기술인원 수 <RequiredMark /><input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.dedicated_technical_count} onChange={(e) => set("dedicated_technical_count", e.target.value)} /><FieldError show={hasMiss(missing, "dedicated_technical_count")} message="필수" /></label>
      </div>
      <div className="mt-4 flex flex-wrap gap-4 text-sm">
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            disabled={!editable}
            checked={form.flags.technical_collaboration_requested}
            onChange={(e) =>
              updateForm((f) => ({
                ...f,
                flags: { ...f.flags, technical_collaboration_requested: e.target.checked }
              }))
            }
          />
          기술 협력 신청
        ...
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            disabled={!editable}
            checked={form.flags.platinum_review_requested}
            onChange={(e) =>
              updateForm((f) => ({
                ...f,
                flags: { ...f.flags, platinum_review_requested: e.target.checked }
              }))
            }
          />
          플래티넘 검토 신청
...
function ContactStep({ form, updateForm, missing, editable }: StepProps) {
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <label className="text-sm">성명 <RequiredMark /><input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.name} onChange={(e) => set("name", e.target.value)} /><FieldError show={hasMiss(missing, "name")} message="필수" /></label>
        <label className="text-sm">직급/직책 <RequiredMark /><input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.position} onChange={(e) => set("position", e.target.value)} /><FieldError show={hasMiss(missing, "position")} message="필수" /></label>
        <label className="text-sm">부서 <RequiredMark /><input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.department} onChange={(e) => set("department", e.target.value)} /><FieldError show={hasMiss(missing, "department")} message="필수" /></label>
        ...
        <label className="text-sm">휴대폰 <RequiredMark /><input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.phone} onChange={(e) => set("phone", formatPhoneInput(e.target.value))} /><FieldError show={hasMiss(missing, "phone")} message="올바른 휴대폰 번호" /></label>
        <label className="text-sm">이메일 <RequiredMark /><input disabled={!editable} className="mt-1 w-full rounded-lg border px-3 py-2" value={c.email} onChange={(e) => set("email", e.target.value)} /><FieldError show={hasMiss(missing, "email")} message="올바른 이메일" /></label>
      </div>
    </div>
  );
}
...
function PeopleStep({ form, updateForm, missing, editable }: StepProps) {
  return (
    <div className="space-y-6">
      ...
      <FieldError show={hasMiss(missing, "engineer")} message="기술 전담인원 최소 2명이 필요합니다." />

      <div>
        <h3 className="font-medium">대표이사 <RequiredMark /></h3>
        {form.people.ceo.map((p, i) => (
          <div key={i} className="mt-2 grid gap-2 sm:grid-cols-3">
            <input disabled={!editable} placeholder="성명" className="rounded-lg border px-3 py-2 text-sm" value={p.name ?? ""} onChange={(e) => updateForm((f) => { const ceo = [...f.people.ceo]; ceo[i] = { ...ceo[i], name: e.target.value, section: "ceo" }; return { ...f, people: { ...f.people, ceo } }; })} />
    ...
    <div>
      <div className="flex items-center justify-between">
        <h3 className="font-medium">
          {title} {required ? <RequiredMark /> : <span className="text-sm font-normal text-slate-500">(선택/조건부)</span>}
        </h3>
        {editable ? (
      ...
      {rows.map((p, i) => (
        <div key={i} className="mt-2 grid gap-2 sm:grid-cols-3">
          <input disabled={!editable} placeholder="성명" className="rounded-lg border px-3 py-2 text-sm" value={p.name ?? ""} onChange={(e) => { const next = [...rows]; next[i] = { ...next[i], name: e.target.value }; onChange(next); }} />
...
function CustomersStep({ form, updateForm, missing, editable }: StepProps) {
  return (
    <div>
      ...
      <FieldError show={hasMiss(missing, "min") || missing.some((m) => m.section === "customers")} message="최소 1건, 고객명/제안상황/사업시기/매출목표 필수" />
      {form.customers.map((row, i) => (
        <div key={i} className="mt-3 grid gap-2 sm:grid-cols-2">
          <input disabled={!editable} placeholder="고객명" className="rounded-lg border px-3 py-2 text-sm" value={row.customer_name ?? ""} onChange={(e) => updateForm((f) => { const customers = [...f.customers]; customers[i] = { ...customers[i], customer_name: e.target.value }; return { ...f, customers }; })} />
          <input disabled={!editable} placeholder="제안 상황" className="rounded-lg border px-3 py-2 text-sm" value={row.proposal_status ?? ""} onChange={(e) => updateForm((f) => { const customers = [...f.customers]; customers[i] = { ...customers[i], proposal_status: e.target.value }; return { ...f, customers }; })} />
          <input disabled={!editable} placeholder="사업 시기" className="rounded-lg border px-3 py-2 text-sm" value={row.business_timing ?? ""} onChange={(e) => updateForm((f) => { const customers = [...f.customers]; customers[i] = { ...customers[i], business_timing: e.target.value }; return { ...f, customers }; })} />
          <input disabled={!editable} placeholder="매출 목표" className="rounded-lg border px-3 py-2 text-sm" value={row.revenue_target ?? ""} onChange={(e) => updateForm((f) => { const customers = [...f.customers]; customers[i] = { ...customers[i], revenue_target: e.target.value }; return { ...f, customers }; })} />
...
function EquipmentStep({ form, updateForm, missing, editable }: StepProps) {
  return (
    <div>
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">장비현황</h2>
        {editable ? (
          <button
            type="button"
            className="text-sm text-blue-700"
      ...
      <FieldError show={missing.some((m) => m.section === "equipment")} message="기술협력/플래티넘 검토 시 필수" />
      {form.equipment.length === 0 ? <p className="mt-2 text-sm text-slate-500">등록된 장비가 없습니다.</p> : null}
...
function EngineersStep({ form, updateForm, missing, editable }: StepProps) {
  return (
    <div>
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">기술인력 프로필</h2>
        {editable ? (
          <button
            type="button"
            className="text-sm text-blue-700"
      ...
      <FieldError show={missing.some((m) => m.section === "engineers")} message="기술협력/플래티넘 검토 시 필수" />
