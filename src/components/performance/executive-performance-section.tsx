...
export function ExecutivePipelineSummarySection({
  stats,
  compact = false
}: {
  ...
  if (!latest) {
    return (
      <section className={compact ? "h-full rounded-2xl border border-slate-200/80 bg-white p-4 shadow-card" : "mt-6 space-y-3"}>
        <SectionHeader title="파트너 파이프라인" href="/dashboard/performance/upload" hrefLabel="업로드" compact={compact} />
        <div className={compact ? "mt-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm font-medium text-slate-700" : "rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm font-medium text-slate-700"}>
    ...
    return (
      <section className="h-full rounded-2xl border border-slate-200/80 bg-white p-4 shadow-card">
        ...
        <p className="mt-2 text-[11px] font-medium text-slate-500">
          기준일 {latest.snapshot_date} · 파트너딜 · 제품매출
        </p>
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
          <PipelineCard
            ...
            supporting={`수주확도 50% 이상 · ${stats.expected_win_count.toLocaleString("ko-KR")}건`}
            share={expectedShare}
            featured
            compact
          />
...
export function ExecutivePipelineTrendSection({
  stats,
  compact = false
}: {
  stats: ExecutivePerformanceStats;
  ...
  if (compact) {
    return (
      <section className="h-full rounded-2xl border border-slate-200/80 bg-white p-4 shadow-card">
        <SectionHeader title="파트너 파이프라인 추이" href="/dashboard/performance" hrefLabel="상세" compact />
        <div className="mt-3">
          {showLineCharts ? (
            <TrendChartCard
              compact
...
function SectionHeader({
  ...
  return (
    <div className={compact ? "flex flex-wrap items-end justify-between gap-3" : "flex flex-wrap items-end justify-between gap-3 border-b border-slate-200 pb-2"}>
      <h2 className={compact ? "text-base font-bold tracking-tight text-slate-950" : "text-xl font-bold tracking-tight text-slate-950"}>
        {title}
      </h2>
      {href ? (
        <Link href={href} className={compact ? "text-xs font-bold text-okestro-700 hover:underline" : "text-sm font-bold text-okestro-700 hover:underline"}>
          {hrefLabel} →
        </Link>
      ) : null}
    </div>
...
function PipelineCard({
  ...
  const baseClass = compact
    ? "flex h-full min-h-[8.75rem] flex-col rounded-xl border border-slate-200/80 bg-white px-4 py-3.5 shadow-sm"
    : PIPELINE_CARD_CLASS;
  ...
  return (
    <div
      className={
        featured
          ? `${baseClass} border-l-[3px] border-okestro-600 bg-gradient-to-br from-white via-okestro-50/40 to-slate-50 ring-1 ring-okestro-100`
          : baseClass
      }
    >
      <p className={compact ? "text-[13px] font-semibold leading-snug text-slate-900" : "text-base font-semibold leading-snug text-slate-900"}>
        {title}
      </p>
      <p
        className={
          compact
            ? featured
              ? "mt-1.5 text-[1.8rem] font-bold tabular-nums leading-none tracking-tight text-slate-950"
              : "mt-1.5 text-[1.65rem] font-bold tabular-nums leading-none tracking-tight text-slate-800"
            : featured
      ...
      {supporting ? (
        <p className={compact ? "mt-auto pt-2 text-[11px] font-medium leading-snug text-slate-600" : "mt-2 text-sm font-medium leading-snug text-slate-600"}>
          {supporting}
        </p>
      ) : null}
      {share != null ? (
        <p className={compact ? "mt-1 text-[11px] font-medium leading-snug text-slate-600" : "mt-1 text-sm font-medium leading-snug text-slate-600"}>
...
function TrendChartCard({
  ...
  return (
    <div className={compact ? "flex min-h-[17rem] flex-col rounded-xl border border-slate-200 bg-slate-50/45 p-3" : "flex min-h-[26rem] flex-col rounded-xl border border-slate-200 bg-white p-3 shadow-sm"}>
      <h3 className={compact ? "mb-1 px-1 text-xs font-semibold text-slate-900" : "mb-1 px-1 text-[15px] font-semibold text-slate-900"}>
        {title}
      </h3>
...
function TrendComparisonCard({
  ...
  if (trend.length === 0) {
    return (
      <div className={compact ? "rounded-xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500" : "rounded-xl border border-dashed border-slate-200 bg-slate-50 p-6 text-sm text-slate-500"}>
        {title}: 데이터 없음
      </div>
    );
  }
  ...
  if (trend.length === 1) {
    ...
    return (
      <div className={compact ? "rounded-xl border border-slate-200 bg-slate-50/45 p-4" : "rounded-xl border border-slate-300/90 bg-white p-5 shadow-sm"}>
        <p className="text-sm font-bold text-slate-950">{title}</p>
        <p className={compact ? "mt-3 text-2xl font-bold tabular-nums tracking-tight text-slate-950" : "mt-3 text-3xl font-bold tabular-nums tracking-tight text-slate-950"}>
  ...
  return (
    <div className={compact ? "rounded-xl border border-slate-200 bg-slate-50/45 p-4" : "rounded-xl border border-slate-300/90 bg-white p-5 shadow-sm"}>
      <p className="text-sm font-bold text-slate-950">{title}</p>
      <div className={compact ? "mt-3 flex flex-wrap items-baseline gap-2 text-xl font-bold tabular-nums text-slate-950" : "mt-3 flex flex-wrap items-baseline gap-2 text-2xl font-bold tabular-nums text-slate-950"}>
