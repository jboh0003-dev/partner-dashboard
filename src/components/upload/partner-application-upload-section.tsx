...
export function PartnerApplicationUploadSection({
  ...
  const peopleSummary = useMemo(() => {
    ...
    return `담당자 ${contract}명 · 영업 ${sales}명 · 기술 ${engineer}명`;
  }, [people]);
  ...
  return (
    <section className={embedded ? "space-y-4" : "mb-6 space-y-4"}>
      ...
      >
        ...
        <div
          className={[
            "flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-10 text-center",
            embedded ? "mt-0" : "mt-4"
          ].join(" ")}
